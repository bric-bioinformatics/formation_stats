---
title: "Examples of practical studies"
format: 
  html: 
    code-fold: true
...
# Dunn post-hoc
dunnTest(glucose ~ diagnosis, data = clinical_data, kw = F) #kw = F allow to not redo the Kruskal-Wallis test
```

```{r, echo=F}
dunnTest(glucose ~ diagnosis, data = clinical_data, kw = F)
```
...
We also have a **Z-score**, which is calculated based on the **ratio between the difference in mean and the difference in standard error**.
...
Here, the question asked seeks to **determine whether there is a difference**. But it does not delve deeper to see **where the difference lies**. 
We can look at the **proportion graphically**. This will give us, in addition to the significance test, an idea of the **direction in which the difference is going**.

```{r, eval=F}
# Observe proportions graphically
ggplot(clinical_data, aes(x = diagnosis, fill = smoking_status)) + 
...
However, unlike ANOVA, no commonly accepted post-hoc for chi-square analysis exists.
